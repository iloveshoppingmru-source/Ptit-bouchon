...
const loadData=()=>{
  // Migrate old keys → force fresh load with new Airbnb photos
  try{localStorage.removeItem("ptitbouchon_v5");localStorage.removeItem("ptitbouchon_v4");}catch{}
  try{const s=localStorage.getItem("ptitbouchon_v7");if(s)return JSON.parse(s);}catch{}
  return JSON.parse(JSON.stringify(DEFAULT));
};
const saveData=(d)=>{try{localStorage.setItem("ptitbouchon_v7",JSON.stringify(d));}catch{}};
